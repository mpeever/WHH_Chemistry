...
\begin{hint}\label{hint:enthalpy-tip}
...
\end{enumerate}
\item in Chemistry, \emph{potential energy} is energy stored in chemical bonds
\item \emph{kinetic energy} is heat
...
\item we think of changes in enthalpy in terms of \emph{energy absorbed} in a reaction, so if $\Delta H > 0$ for a given reaction, the reaction is \emph{endothermic} (see Hint \ref{hint:enthalpy-tip}, p. \pageref{hint:enthalpy-tip})
...
\item a pile of dirt sitting on a field has much less entropy than the same pile washed with rain, spread out over the dirt
