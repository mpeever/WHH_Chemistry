...
\item electrical charges are with \emph{positive} or \emph{negative}\footnote{These aren't value judgments, we just use them to categorize charge, and it turns out the math works out nicely. Don't get too hung up on them: negative charges aren't worse than positive charges, they're just charged oppositely.}
...
\item note carefully the critique of Rutherford's model\cite[p. 213]{wile-chem-2}: even in Rutherford's time, it was known that a charged particle moving in a circle would lose energy in the form of emitted light, so that the electron would quickly fall into the atom's center
\end{itemize}
...
\begin{itemize}
\item waves are a disturbance in a medium that carries energy while the medium does not\footnote{This can be confusing to students, because the waves we see in real life aren't always the best example of waves. Surf, for example, is actually a \emph{decaying} wave.}
\item waves have a:
\begin{itemize}
\item wavelength ($\lambda$) (measured in meters, centimeters, etc.)
\item frequency ($f$ or $\nu$) (measured in Hertz (Hz))
\item amplitude ($A$)
\item a speed ($v$) (measured in ``meters per second" ($\frac{m}{s}$))
\end{itemize}
...
\item in 1908, Albert Einstein won the Nobel Prize for his work on the photoelectric effect\footnote{Not, surprisingly, for his work on Relativity.}
...
\item Bohr's model really only works perfectly for \ce{H} atoms, but it's the basis of what we now consider the ``current" model
\end{itemize}
...
This gets complicated quickly, and it can be easy to get lost in this one.} 
...
\item there are three $s$ orbitals: $1s$, $2s$, and $3s$
...
\item there is no $1p$ orbital, so there's not a $p$ orbital in the innermost energy level
...
\item so there are three $2p$ orbitals, three $3p$ orbitals, 3 $4p$ orbitals, etc.
...
\item there is no $1d$ or $2d$ orbital, so there are no $d$ orbitals below the third energy level
\item there are 5 $d$ orbitals per energy level
...
\item the lowest energy level is $1$, and the lowest energy orbital is $1s$ (actually, level 1 only has one orbital)
